...
show_usage() {
    ...
    echo ""
    echo "Logging:"
    echo "  Each service logs to its own directory: logs/[service_name]/"
    echo "  Log files are named: [service_name]_YYYYMMDD_HHMMSS.log"
    echo "  Old log files are automatically cleaned up (keeps last 10 per service)"
    echo ""
    echo "Examples:"
...
log_info "Log Directory: $(dirname "$LOG_FILE")"
